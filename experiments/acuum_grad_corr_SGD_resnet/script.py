...
class CorrResNetPLModel(ResNetPLModel):
    def on_before_optimizer_step(self, optimizer):
        ...
        round_number = curr_round + (current_epoch + batch_idx)/client_epochs_per_round / batch_count
...
if __name__ == '__main__':
    ...
    batch_size = 7_500  # 3 batches per w = 73000/2worker/13000  -  np.ceil(len(dataset[0])/batch_size/2)
    ...
    # *****************
    sim = FLSimulator(
        pl_model=model, num_agents=2, communication_rounds=50, client_epochs_per_round=client_epochs_per_round,
