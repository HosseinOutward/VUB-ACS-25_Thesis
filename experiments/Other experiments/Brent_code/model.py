...
class EncoderDecoder(nn.Module):
    ...
    def __init__(self, input_dim=1, layers=3, hidden_dim=100, code_size=4, marginal=True):

        super().__init__()

        self.code_size = code_size

        self.encoder = Encoder(input_dim=input_dim, code_size=code_size, layers=layers, hidden_dim=hidden_dim)
        self.decoder = Decoder(code_size=code_size, output_dim=input_dim, layers=layers, hidden_dim=hidden_dim)
        if marginal is True:
    ...
    def forward(self, x, y, tau=1.0):
        ...
        return reconstruct, bin, soft_code, prior
