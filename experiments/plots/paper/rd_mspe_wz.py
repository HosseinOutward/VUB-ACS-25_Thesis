...
def conditional_rd(
    x: np.ndarray, y: Optional[np.ndarray] = None, W: Optional[np.ndarray] = None,
    u_idx: Optional[np.ndarray] = None, n_buckets: int = 8, n_x_bins: int = 64, n_recon: int = 64,
    s_grid: Optional[np.ndarray] = None, D_targets: Optional[np.ndarray] = None,
    dist: str = "mspe", mape_percent: bool = False, eps: float = 1e-6,
    max_iter: int = 400, tol: float = 1e-6, rng_seed: int = 42,
) -> Dict[str, Any]:
    def _kmeans(
            y: np.ndarray, n_clusters: int, n_init: int = 5, max_iter: int = 100,
    ...
    def _empirical_p_x_given_u_from_soft(
            x_bin_idx: np.ndarray, W: np.ndarray, n_bins: int) -> Tuple[np.ndarray, np.ndarray]:
    ...
    def _empirical_p_x_given_u_from_hard(x_bin_idx: np.ndarray, u_idx: np.ndarray, K: int, M: int) -> Tuple[
        ...
        p_u = counts.sum(axis=1);
        p_u /= p_u.sum()
        p_x_given_u = counts + 1e-16
        p_x_given_u /= p_x_given_u.sum(axis=1, keepdims=True)
        return p_x_given_u, p_u

    def _distances(x_grid: np.ndarray, r_grid: np.ndarray, kind: str = "mspe", eps: float = 1e-6,
                   mape_percent: bool = False) -> np.ndarray:
    ...
    def _ba_bucket(p_x: np.ndarray, dmat: np.ndarray, s: float, max_iter: int = 500, tol: float = 1e-7) -> Tuple[
...
# target.shape == (vec_size,); si.shape == (vec_size, si_count);
# res_dict = conditional_rd(
#     dist="mape", mape_percent=True,
#     x=target, y=si,
#     s_grid=np.logspace(-3,3,20), D_targets=np.array([0.05, 0.4]),
#     n_buckets=8, n_x_bins=56, n_recon=56, )
# np.savez_compressed(f'wz_bound_ai-{agent_id}_ri-{round_id}.npz', D=res_dict['D'], R=res_dict['R'])
